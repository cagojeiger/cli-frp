...
class ServerConfig(BaseModel):
    ...
    kcp_bind_port: int | None = Field(None, ge=1, le=65535, description="KCP protocol port")
    ...
    auth_token: str | None = Field(None, min_length=8, description="Authentication token")
    ...
    def validate_auth_token(cls, v: str | None) -> str | None:
        ...
        if v is not None:
            if len(v) < MIN_AUTH_TOKEN_LENGTH:
                raise ValueError(f"Auth token must be at least {MIN_AUTH_TOKEN_LENGTH} characters")
...
class DashboardConfig(BaseModel):
    ...
    def validate_password(cls, v: str | None) -> str | None:
        ...
        if v is not None and len(v) < MIN_PASSWORD_LENGTH:
            raise ValueError(f"Dashboard password must be at least {MIN_PASSWORD_LENGTH} characters")
        return v
...
class SSLConfig(BaseModel):
    ...
    use_letsencrypt: bool = Field(default=False, description="Use Let's Encrypt for certificates")
    ...
    letsencrypt_challenge_type: str = Field(default="http", description="Challenge type (http/dns)")

    auto_renew: bool = Field(default=True, description="Auto-renew certificates")
    renew_days_before: int = Field(default=30, ge=1, le=90, description="Days before expiry to renew")
    ...
    def validate_ssl_config(self) -> 'SSLConfig':
        ...
        if has_manual_certs:
            if not (self.cert_file and self.key_file):
                raise ValueError("Both cert_file and key_file are required for manual certificates")

        if self.use_letsencrypt:
            if not self.letsencrypt_email:
                raise ValueError("Let's Encrypt email is required when using Let's Encrypt")
...
class CompleteServerConfig(BaseModel):
    ...
    @classmethod
    def load_from_file(cls, _path: Path) -> 'CompleteServerConfig':
        """Load configuration from TOML file (placeholder for future implementation)"""
        return cls(
            server=ServerConfig(
                kcp_bind_port=None,
                auth_token=None,
