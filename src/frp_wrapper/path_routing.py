...
class PathPattern:
    ...
    def conflicts_with(self, other: 'PathPattern') -> bool:
        ...
        if self.is_wildcard or other.is_wildcard:
            return (self.matches(other.pattern.replace("*", "test")) or 
                   other.matches(self.pattern.replace("*", "test")))
...
class PathConflictDetector:
    ...
    def check_conflict(self, new_path: str, existing_paths: List[str]) -> Optional[str]:
        ...
        for existing_path in existing_paths:
            ...
            if new_pattern.conflicts_with(existing_pattern):
                return f"Path '{new_path}' conflicts with existing path '{existing_path}'"
        
        return None
    
    def detect_conflicts(self, new_path: str, new_tunnel_id: str) -> List[PathConflict]:
        """Detect all conflicts for a new path.
        
        Args:
            new_path: New path to check
            new_tunnel_id: ID of tunnel requesting the path
